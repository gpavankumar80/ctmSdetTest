...
public class Electricity extends ActionCommands
{
	  int urlCol = 0, urlRow = 1, pCodeCol = 1, pCodeRow = 1, emailCol = 2, emailRow = 1;
	  @Test
	  public void ElectricityComparision() 
	  {
			...
			click(Supplier.electicity_Compare);
			//Click on the BritishGas label under "who supplies your electricity"
			click(Supplier.suppliesElectricity_BritishGas);
			...
			dropDown(Energy.electricityTariff_dropDown, Energy.electricityTariff_dropDownList);
			// check the "yes" check box for Economy 7 meter
			click(Energy.electricy_Economy7Meter);
			// select the 2nd option in the pay your electricity list box
			dropDown(Energy.electicity_PayForElectricity_DropDown, Energy.electricity_PayForElectricity_DropDownList);
			// check the "yes" check box for "is electricity is your main source of heating" 
			click(Energy.electricity_MainSourceOfHeating_Yes);
			//Enter text in "Economy7DayUsage" text box
			sendText(Energy.electricity_Economy7DayUsage, "100");
			//Enter text in "Economy7NightUsage" text box
			sendText(Energy.electricity_Economy7NightUsage,"50");
